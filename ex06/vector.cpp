...
float Vector::operator[](unsigned int index){
 ...
 const float Vector::operator[](unsigned int index) const{
  return this->values[index];
 }
...
Vector *cross_product(Vector &u, Vector &v){
  ...
  out->values.push_back((u[1] * v[2]) - (u[2] * v[1]));
  out->values.push_back(((u[0] * v[2]) - (u[2] * v[0])) * -1);
  out->values.push_back((u[0] * v[1]) - (u[1] * v[0]));
  return out;
}